...
db.Restaurants.find({ area: "Bronx" });
...
db.Restaurants.find({ area: "Bronx" }).skip(5).limit(5);
...
db.Restaurants.find({ longitude: { $lt: -95.754168 } });
...
db.Restaurants.find({
  cuisine: { $ne: "American" },
  "grades.score": { $gt: 70 },
  "address.coord.longitude": { $lt: -65.754168 },
});
...
db.Restaurants.find(
  { "address.coord.1": { $regex: {$gt: 42, $lt: 52}} },
  {
    restaurant_id: 1,
    name: 1,
    address: 1,
...
db.Restaurants.find(
  ...
  },
  {
    name: 1,
    borough: 1,
    "address.coord.longitude": 1,
    "address.coord.latitude": 1,
    cuisine: 1,
  }
);
